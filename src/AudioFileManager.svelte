<script>
  ...
  async function loadTransientsFromJson(file) {
    try {
      ...
      if (data.masterSong && projectTree.masterSong) {
        ...
        projectTree.masterSong.included =
          data.masterSong.included !== undefined
            ? data.masterSong.included
            : true;
        projectTree.masterSong.analyzing = false;
        console.log(
          `📂 Loaded ${projectTree.masterSong.transients.length} master transients`,
        );
      }

      // Match stems by type and apply transients
      data.stems.forEach((savedStem) => {
        const currentStem = projectTree.stems.find(
          (s) => s.type === savedStem.type,
        );
        if (currentStem) {
          currentStem.transients = savedStem.transients || [];
          currentStem.duration = savedStem.duration || 0;
          currentStem.included =
            savedStem.included !== undefined ? savedStem.included : true;
          currentStem.visible =
            savedStem.visible !== undefined ? savedStem.visible : true;
          currentStem.analyzing = false;
          console.log(
            `📂 Loaded ${currentStem.transients.length} transients for ${currentStem.type}`,
          );
        }
      });
  ...
  function inferStemType(filename) {
    ...
    // Backing Vocals
    if (name.includes("backing") && name.includes("vocal"))
      return "backing-vocals";
    if (name.includes("back") && name.includes("vocal"))
      return "backing-vocals";
    if (name.includes("harmony") || name.includes("choir"))
      return "backing-vocals";
    ...
    // Percussion
    if (name.includes("percussion") || name.includes("perc"))
      return "percussion";
    if (name.includes("shaker") || name.includes("tambourine"))
      return "percussion";
  ...
  async function handleStemUpload(event) {
    ...
    for (const file of files) {
      ...
      const stem = {
        ...
        included: true, // Controls transient visibility now
        visible: true,
        muted: false,
        soloed: false,
        analyzing: true,
    ...
    for (const stem of stemsToProcess) {
      try {
        ...
        if (stemToUpdate) {
          ...
          // Keep file for playback in AudioTimeline
  ...
  async function detectMasterTransients() {
    ...
    if (
      masterDetectionInProgress ||
      (projectTree.masterSong.transients &&
        projectTree.masterSong.transients.length > 0)
    ) {
      console.log(
        "🔄 Master transients already detected or analyzing, skipping...",
      );
      return;
    }
    ...
    try {
      ...
      const result = await analyzeAudioForTransients(
        projectTree.masterSong.file,
        "master",
      );
      ...
      console.log(
        `✅ Found ${result.transients.length} transients in master track`,
      );
  ...
  async function analyzeAudioForTransients(file, stemType) {
    ...
    try {
      ...
      console.log(
        `📊 Audio data: ${rawData.length} samples, ${sampleRate}Hz, ${duration.toFixed(2)}s`,
      );
      ...
      const skipFactor = Math.max(
        1,
        Math.round((101 - settings.density) * 0.2),
      );
      ...
      console.log(
        `🔧 Detection params for ${stemType}: density=${settings.density}, sensitivity=${settings.sensitivity}, minSpacing=${settings.minSpacing}s`,
      );
      ...
      console.log(
        `🎵 Detection complete: Found ${transients.length} transients in ${duration.toFixed(2)}s`,
      );
  ...
  function toggleMasterIncluded() {
    if (projectTree.masterSong) {
      ...
      console.log(
        `🎵 Master track ${projectTree.masterSong.included ? "included" : "excluded"}`,
      );
      updateCombinedTransients();
      // Force reactivity update
      projectTree = { ...projectTree };
  ...
  function toggleStemIncluded(stemId) {
    ...
    if (stem) {
      ...
      console.log(
        `🎵 Stem ${stem.type} transients ${stem.included ? "shown" : "hidden"}`,
      );
      // Removed updateCombinedTransients() - waiting for Apply button
      // Force reactivity update
      projectTree = { ...projectTree };
    }
  }
  ...
  function toggleStemMute(stemId) {
    ...
    if (stem) {
      ...
      // If muting, we might want to unmute solo if it was soloed?
      // Standard DAW: Mute is independent.
      updateCombinedTransients(); // Dispatch updates
      projectTree = { ...projectTree };
    }
  }

  function toggleStemSolo(stemId) {
    const stem = projectTree.stems.find((s) => s.id === stemId);
    if (stem) {
  ...
  function updateCombinedTransients() {
    ...
    const visibleStems = projectTree.stems.filter(
      (s) => s.visible && s.included,
    );
    ...
    // Dispatch master update to ensure inclusion state is propagated
    if (projectTree.masterSong) {
      dispatch("masterSelected", projectTree.masterSong);
    }
  }
...
<div class="audio-file-tree">
  ...
  <div class="json-controls">
    <button
      class="json-btn save-btn"
      onclick={() => saveTransientsToJson(projectTree)}
    >
      💾 Save Transients
    </button>
  ...
  <div class="apply-controls" style="margin-bottom: 15px;">
    <button class="apply-btn" onclick={() => updateCombinedTransients()}>
      ✨ Apply Filters
    </button>
  </div>
    ...
    <div class="tree-header">
      <button class="expand-btn" onclick={() => toggleSection("master")}>
        <span class="expand-icon"
          >{projectTree.expanded.master ? "▼" : "▶"}</span
        >
        <span class="section-title"
          >Master ({projectTree.masterSong ? "1" : "0"})</span
        >
      </button>
      {#if !projectTree.masterSong}
            ...
            <div class="master-controls">
              <div class="track-controls">
                <label class="toggle-switch">
                  ...
                  <span
                    class="toggle-slider"
                    style="background-color: {projectTree.masterSong.included
                      ? '#ffef00'
                      : '#555'};"
                  ></span>
                ...
                <span
                  class="transient-count"
                  style="color: #ffef00; border-color: #ffef00;"
                >
  ...
  <div class="tree-section">
    <div class="tree-header">
      <button class="expand-btn" onclick={() => toggleSection("stems")}>
        <span class="expand-icon"
          >{projectTree.expanded.stems ? "▼" : "▶"}</span
        >
      ...
      <div class="tree-content">
        {#each stemTypes as stemType}
          ...
          <div
            ...
            style="border-left: 3px solid {stemType.color}; background-color: {stem
              ? stemType.bgColor
              : 'transparent'}"
          >
            <div class="stem-header">
              <div class="stem-identity">
                <span
                  class="stem-dot"
                  style="background-color: {stemType.color}"
                ></span>
                <span
                  class="stem-name"
                  style={stem ? `color: ${stemType.color}` : ""}
                  >{stemType.name}</span
                >
              </div>
              ...
              {#if stem}
                <div class="stem-controls">
                  ...
                  <button
                    ...
                    class:active={stem.soloed}
                    onclick={() => toggleStemSolo(stem.id)}
                    title="Solo">S</button
                  >

                  <label class="toggle-switch compact" title="Show Transients">
                    <input
                      type="checkbox"
                    ...
                    <span
                      class="toggle-slider"
                      style="background-color: {stem.included
                        ? stem.color
                        : '#555'};"
                    ></span>
                  </label>
                  ...
                  <span
                    class="transient-count"
                    style="color: {stem.color}; border-color: {stem.color};"
                  >
            ...
            {#if stem}
              ...
              <div
                class="transient-timeline"
                style="background-color: {stemType.bgColor}"
              >
                <div class="timeline-bar">
                  {#each stem.transients as transient}
                    <div
                      class="transient-marker"
                      style="left: {(transient / (stem.duration || 180)) *
                        100}%; background-color: {stemType.color};"
                  ...
                  {#if stem.transients?.length > 0}
                    <div
                      style="position: absolute; top: -12px; left: 2px; font-size: 8px; color: {stemType.color}; background: rgba(0,0,0,0.7); padding: 1px 3px; border-radius: 2px;"
                    >
...
<style>
  ...
  .apply-btn {
    width: 100%;
    background: linear-gradient(135deg, #00b8a9 0%, #00f2ea 100%);
    border: none;
    color: #000;
    font-weight: bold;
    padding: 8px;
  ...
  .apply-btn:hover {
    transform: translateY(-1px);
    box-shadow: 0 4px 15px rgba(0, 184, 169, 0.5);
  }

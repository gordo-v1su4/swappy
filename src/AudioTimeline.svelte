<script>
  ...
  function calculateAudioReactiveSpeed() {
    ...
    try {
      ...
      const smoothedEnergy =
        audioEnergyBuffer.reduce((sum, val) => sum + val, 0) /
        audioEnergyBuffer.length;
  ...
  onMount(() => {
    ...
    wavesurfer.on("audioprocess", () => {
      const now = Date.now();
      if (now - lastAudioprocessTime > 16) {
        // Throttle to ~60 FPS for better precision
  ...
  function createMarker() {
    ...
    if (markers.length === 0) {
      ...
      if (insertIndex === 0) {
        ...
        // Between two markers
        markerNumber =
          (sortedMarkers[insertIndex - 1].data.number +
            sortedMarkers[insertIndex].data.number) /
          2;
      }
    }
  ...
  async function analyzeAudio() {
    ...
    try {
      ...
      if (duration <= 60) {
        ...
        const remainingTime =
          duration -
          (introLength + outroLength + bridgeLength + preChorusLength);
        verseLength = remainingTime * 0.6;
        chorusLength = remainingTime * 0.4;
      } else {
        // Long track (> 3 min)
  ...
  export function handleTransientsUpdated(data) {
    combinedTransients = data.transients || [];
    console.log(
      `📥 Received ${combinedTransients.length} combined transients from file manager`,
    );
    updateTransients();
  }

  // Update transients display based on current analysis controls
  export function updateTransients() {
    ...
    try {
      ...
      console.log(
        `✅ Applied filters: ${transientMarkers.length} transients displayed (from ${combinedTransients.length} total)`,
      );
    } catch (error) {
  ...
  function filterCombinedTransients(transients) {
    ...
    for (const source in transientsByStem) {
      ...
      stemTransients = stemTransients.filter(
  ...
  function createTransientMarker(
    time,
    index,
    total,
    customColor = null,
  ...
  function checkTransientHit(time) {
    ...
    const isHit = transientMarkers.some(
      (marker) => Math.abs(marker.start - time) < hitTolerance,
    );
    ...
    if (isHit && !isTransientHit) {
      ...
      if (speedRampEnabled && speedRampTrigger === "transients") {
        ...
        console.log(
          "🔇 Speed ramp conditions not met. Enabled:",
          speedRampEnabled,
          "Trigger:",
          speedRampTrigger,
        );
  ...
  function triggerSpeedRamp() {
    ...
    console.log(
      "🎯 Speed ramp triggered! Intensity:",
      speedRampIntensity + "%",
    );
    ...
    if (randomChance < speedRampIntensity) {
      ...
      console.log(
        "🚀 Starting speed ramp curve to",
        targetSpeed.toFixed(2) + "x over",
        speedRampDuration,
        "seconds",
      );
      console.log(
        "📈 Ramp up:",
        speedRampTransitionTime + "s, Hold:",
        (speedRampDuration - 2 * speedRampTransitionTime).toFixed(1) +
          "s, Ramp down:",
        speedRampTransitionTime + "s",
      );
  ...
  // export { handleTransientsUpdated, updateTransients }; // Removed in favor of direct export
  ...
  $effect(() => {
    ...
    if (
      (currentTransientCount !== lastTransientCount ||
        currentUserMarkersCount !== lastUserMarkersCount) &&
...
<div class="audio-timeline">
  ...
  <div class="waveform-container" bind:this={container} {id}></div>
  <div
  ...
  <div class="region-controls">
    <button class="button" onclick={createRegion} disabled={!isLoaded}>
      Create Region
    </button>

    <button class="button" onclick={playRegion} disabled={!activeRegion}>
      Play Region
    </button>

    <button class="button" onclick={deleteRegion} disabled={!activeRegion}>
      Delete Region
    </button>

    <button class="button" onclick={exportRegion} disabled={!activeRegion}>
      Export Region
    </button>
      ...
      <div class="analysis-sliders">
        <div class="slider-group">
          <label for="density-slider"
            >Density <span class="slider-value">{transientDensity}%</span
            ></label
          >
          <input
            id="density-slider"
        ...
        <div class="slider-group">
          <label for="randomness-slider"
            >Randomness <span class="slider-value">{transientRandomness}%</span
            ></label
          >
          <input
        ...
        <div class="slider-group">
          <label for="sensitivity-slider"
            >Sensitivity <span class="slider-value"
              >{transientSensitivity}%</span
            ></label
          >
          <input
        ...
        <div class="slider-group">
          <label for="spacing-slider"
            >Min Spacing <span class="slider-value"
              >{transientMinSpacing.toFixed(2)}s</span
            ></label
          >
          <input
            id="spacing-slider"
      ...
      <div class="analysis-header">
        ...
        <div class="header-controls">
          <div
            ...
            style="--speed-intensity: {speedRampActive
              ? Math.min(
                  1.0,
                  Math.max(
                    0.0,
                    (currentSpeedRamp - 1.0) / (speedRampMaxSpeed - 1.0),
                  ),
                )
              : 0}"
            title="Speed: {currentSpeedRamp.toFixed(1)}x"
          ></div>
          <button
            class="button toggle-btn {speedRampEnabled ? 'active' : ''}"
            onclick={() => (speedRampEnabled = !speedRampEnabled)}
          >
            {speedRampEnabled ? "ON" : "OFF"}
          </button>
        </div>
      </div>
      ...
      {#if speedRampEnabled}
        <div class="speed-ramp-controls">
          <div class="control-group">
            <label for="speed-max-slider">
              <span>Max Speed</span>
              <span class="value-badge">{speedRampMaxSpeed.toFixed(1)}x</span>
            </label>
            <div class="slider-container">
              <input
                id="speed-max-slider"
              ...
              <div
                ...
                style="width: {((speedRampMaxSpeed - 1.1) / 3.9) * 100}%"
              ></div>
            </div>
          </div>

          <div class="control-group">
            <label for="speed-intensity-slider">
              <span>Intensity</span>
              <span class="value-badge">{speedRampIntensity}%</span>
            </label>
            <div class="slider-container">
              <input
              ...
              <div
                class="slider-track-fill"
                style="width: {speedRampIntensity}%"
              ></div>
            </div>
          </div>

          <div class="control-group">
            <label for="speed-duration-slider">
              <span>Duration</span>
              <span class="value-badge">{speedRampDuration.toFixed(1)}s</span>
            </label>
            <div class="slider-container">
              <input
              ...
              <div
                class="slider-track-fill"
                style="width: {((speedRampDuration - 1.0) / 11.0) * 100}%"
              ></div>
            </div>
          </div>
        ...
        <div class="structure-overlay">
          {#each songStructure as section}
            <div
              ...
              title="{section.name}: {formatTime(section.start)} - {formatTime(
                section.end,
              )}"
            >
          ...
          {#each markers as marker}
            <div
              ...
              style="left: {(marker.start / duration) *
                100}%; background-color: {marker.color};"
            ></div>
            <div
              class="marker-item"
              style="left: {(marker.start / duration) *
                100}%; border-color: {marker.color};"
            >
              <span class="marker-number"
                >{marker.data && marker.data.number
                  ? marker.data.number.toFixed(1)
                  : ""}</span
              >
              <span class="marker-timestamp"
                >{marker.data && marker.data.timestamp
                  ? marker.data.timestamp
                  : formatTime(marker.start)}</span
              >
              <button
                class="marker-delete"
          ...
          {#each transientMarkers as marker}
            <div
              ...
              style="left: {(marker.start / duration) *
                100}%; background-color: {marker.color}; opacity: 0.6;"
              title="Transient at {marker.data && marker.data.timestamp
                ? marker.data.timestamp
      ...
      <div class="structure-sections">
        {#each songStructure as section}
          <div class="section-item">
            ...
            >
              {section.name} ({formatTime(section.start)} - {formatTime(
                section.end,
              )})
            </button>
          </div>
...
<style>
  ...
  .marker-button {
    background: rgba(75, 94, 171, 0.3);
    border-color: rgba(75, 94, 171, 0.5);
    color: #a0b0ff;
  }

  .marker-button:hover {
    background: rgba(75, 94, 171, 0.5);
    color: white;
  }
  ...
  .detect-button {
    width: 100%;
    margin-top: 20px;
    background: rgba(233, 184, 60, 0.2);
    border-color: rgba(233, 184, 60, 0.4);
    color: #e9b83c;
  }

  .detect-button:hover {
    background: rgba(233, 184, 60, 0.4);
    color: #fff;
  }
